...
var Input = objectType({ situation: stringType().trim().min(1, "Please describe your situation").max(2e3) });
...
var askAssistant = createServerFn({ method: "POST" }).validator((d) => Input.parse(d)).handler(askAssistant_createServerFn_handler, async ({ data }) => {
	const apiKey = process.env.GEMINI_API_KEY;
	if (!apiKey) throw new Error("The assistant is not configured yet.");
	const { data: orgs } = await createClient(process.env.SUPABASE_URL, process.env.SUPABASE_PUBLISHABLE_KEY, { auth: {
		storage: void 0,
		persistSession: false,
		autoRefreshToken: false
	} }).from("aid_organizations").select("id,name,category,description,amount_label,region,tags,website");
	const activeOrgs = orgs && orgs.length > 0 && !orgs.some((o) => o.name.includes("FEMA")) ? orgs : [
		{
	...
	const catalog = activeOrgs.map((o) => `- [${o.id}] ${o.name} (${o.category}, ${o.region}) — ${o.description} Tags: ${o.tags?.join(", ")}`).join("\n");
...
- Always respond with valid JSON only, matching: {"message": string, "recommendedIds": string[]}`;
	...
	if (res.status === 429 || res.status === 503) throw new Error("The assistant is busy right now. Please try again in a moment.");
	if (res.status === 402) throw new Error("AI usage limit reached. Please add credits to keep using the assistant.");
	if (res.status === 403) throw new Error("Your GEMINI_API_KEY has been disabled/blocked by Google as leaked. Please generate a new key at Google AI Studio and update the GEMINI_API_KEY in your local .env file.");
	if (!res.ok) {
	...
	const recommended = activeOrgs.filter((o) => ids.includes(o.id)).map((o) => ({
		id: o.id,
		name: o.name,
		category: o.category,
	...
	return {
		message: parsed.message?.trim() || "Here are some programs that may be able to help with your situation.",
		recommended
	};
});
//#endregion
